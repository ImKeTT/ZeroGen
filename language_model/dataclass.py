...
class Data:
    def __init__(self, model_name, train_path, dev_path, test_path, max_len, 
        ...
        self.train_token_list, self.train_token_id_list = self.process_one_file_visnews(train_path)
        self.dev_token_list, self.dev_token_id_list = self.process_one_file_visnews(dev_path)
        self.test_token_list, self.test_token_id_list = self.process_one_file_visnews(test_path)
    ...
    def process_one_file_visnews(self, path):
        print ('Processing {}'.format(path))
        with open(path, 'r') as f:
            item_list = f.readlines()
        lines = []
        for item in item_list:
            captions_list = item.split("\t")[-1]
            lines.append(captions_list.strip())
